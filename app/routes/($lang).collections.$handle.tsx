...
export async function loader({ request, params, context }: LoaderFunctionArgs) {
    ...
    let queryVars: any = { handle };
    if (before) {
        queryVars = { ...queryVars, last: limit, before };
    } else if (after) {
        queryVars = { ...queryVars, first: limit, after };
    } else {
        queryVars = { ...queryVars, first: limit };
    }
  ...
  `;
    ...
    try {
        ...
        const products = shopifyData.collection.products.edges.map((edge: any) => formatProduct(edge.node));
        const pageInfo = shopifyData.collection.products.pageInfo;

        let bannerImage = shopifyData.collection.image?.url;
        if (!bannerImage && products.length > 0) {
            bannerImage = products[0].image;
        }
...
export default function CollectionRoute() {
    const { collection, products, pageInfo, detectedCurrency, locale } = useLoaderData<typeof loader>();
    ...
    const handleNext = () => {
        if (pageInfo.hasNextPage && pageInfo.endCursor) {
            navigate(`/${locale}/collections/${collection.handle}?after=${pageInfo.endCursor}`);
        }
    };

    const handlePrevious = () => {
        if (pageInfo.hasPreviousPage && pageInfo.startCursor) {
            navigate(`/${locale}/collections/${collection.handle}?before=${pageInfo.startCursor}`);
        }
    };
            ...
            <main style={{ flex: 1, paddingBottom: "var(--spacing-2xl)" }}>
                ...
                {collection.bannerImage && (
                    <div style={{
                        ...
                        <div style={{
                            ...
                            <h1 className="title-main" style={{ color: "white", margin: 0, textShadow: "0 2px 4px rgba(0,0,0,0.5)" }}>
                                {collection.title}
                            </h1>
                        </div>
                    </div>
                ...
                <div className="container">
                    ...
                    {products.length > 0 ? (
                        ...
                        <div style={{ textAlign: "center", padding: "40px" }}>No products found in this collection.</div>
                    )}
                </div>
            </main>
